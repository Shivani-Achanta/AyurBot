...
const MyAppointmentScreen = () => {
  ...
  const fetchAppointments = async () => {
    const patientEmail = location.state.patientEmail; // Access patientEmail from location state
    try {
      const response = await axios.post(
        "http://localhost:5000/api/fetchappointments",
        {
          patientEmail: patientEmail, // Use patientEmail obtained from state
        }
      );
      const appointmentsWithDoctorName = await Promise.all(
        response.data.map(async (appointment) => {
          const doctorInfo = await fetchDoctorInfo(appointment.doctor);
          return { ...appointment, doctorName: doctorInfo.name };
        })
      );
      setAppointments(appointmentsWithDoctorName);
    } catch (error) {
